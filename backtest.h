...
Matrix PortfolioCreator(vector<Intrix> ID_LHA, Tensor3 beta_LHA, const Matrix& Rs,
        ...
        const Vector& betaCond, const string& method, const bool& beta_weighted)
{
    ...
    Matrix LHA_PortData = PortfolioCreator(ID_LHA, beta_LHA, Rs, riskFree, iPeriods, iDates, Dates, post_yrMin,
...
void BackTest_exotic(const string& incr, string folderName, const string& dataFolderName, Matrix Rs,
        const Vector& Cond, const string& method, string effect)
{
    //std::this_thread::sleep_for(std::chrono::seconds(5)); //If needed for synchronising threads, (easy workaround)
    auto start = std::chrono::system_clock::now();
    ...
    auto [ID_LHA, beta_LHA] = StockSelector_seeking(Data, Data_Advanced, Cond, effect);   //MC strategy

    //Create portfolios
    Matrix LHA_PortData = PortfolioCreator(ID_LHA, beta_LHA, Rs, riskFree, iPeriods, iDates, Dates, post_yrMin,
            post_yrMax, method, false);
